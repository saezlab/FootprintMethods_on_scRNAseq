...
regulon = read_csv("data/regulons/dorothea/dorothea_regulon_human_v1.csv") %>%
...
#### Run SCENIC
##### Build protocol dependent regulatory networks
```{r}
# path = "output/hca_data/scenic/complete/CEL-Seq2/int/2.6_regulons_asGeneSet.Rds"
scenic_regulons = list.files("output/hca_data/scenic/complete", recursive = T, 
                             full.names = T, pattern = "2.6_") %>%
  map_dfr(function(path) {
    technology = path %>% str_split(pattern = "/") %>% pluck(1,5)
    readRDS(path) %>%
      enframe("tf", "target") %>%
      mutate(technology = technology) %>%
      filter(!str_detect(tf, "extended")) %>%
      unnest(target) %>%
      select(technology, tf, target)
  }) %>%
  nest(-technology, .key="regulons") %>%
  mutate(technology = factor(
    technology, levels = c("Chromium","inDrop","C1HT-medium","C1HT-small",
                           "CEL-Seq2","ddSEQ","Drop-Seq","ICELL8","MARS-Seq",
                           "Chromium (sn)","Quartz-Seq2","SCRB-Seq",
                           "Smart-Seq2")))

saveRDS(scenic_regulons, "output/hca_data/scenic/complete/scenic_regulons.rds")
```
...
genesets = scenic_regulons %>%
  transmute(technology, geneset = regulons %>% map(function(regulons) {
    regulons %>%
      group_by(tf) %>%
...
input = df %>%
  inner_join(genesets, by="technology")


ss_scenic_scores = input %>%
  transmute(technology, ss_scenic_result = pmap(., .f= function(norm, geneset, ...) {
    obj = AUCell_buildRankings(norm, nCores=4, plotStats = F, verbose = F) %>%
...
saveRDS(ss_scenic_scores, "output/hca_data/scenic/complete/scenic_result.rds")
```
...
scenic = readRDS("output/hca_data/scenic/complete/scenic_result.rds") %>%
  mutate(tool = "scenic") %>%
  transmute(technology,tool, activity = ss_scenic_result %>% map(function(scenic) {
    scenic %>%
      data.frame(check.names = F, stringsAsFactors = F) %>%
      rownames_to_column("feature") %>%
      as_tibble() %>%
...
summarized_footprint_scores = bind_rows(progeny,viper, paucell, daucell, metaviper, scenic) %>%
  unnest() %>%
...
saveRDS(summarized_footprint_scores, 
        "output/hca_data/summarized_footprint_scores/summarized_footprints_scores.rds")
write_csv(summarized_footprint_scores, 
          "output/hca_data/summarized_footprint_scores/summarized_footprints_scores.csv")
```
...
# SCENIC -----------------------------------------------------------------------
scenic = readRDS("output/hca_data/scenic/scenic_result.rds") %>%
  mutate(class = "scenic") %>%
  dplyr::rename(mat = ss_scenic_result)
...
hrchy = readRDS("output/hca_data/umap/cluster_hierarchy.rds") %>%
  unnest(cluster_vec) %>%
  select(-id) %>%
  spread(hrchy_level, label)

df = list.files("output/hca_data/umap/umap_input", full.names = T) %>%
...
cor = sil_res %>%
  ...
  mutate(label = fct_relevel(label, "Neg. control", after=Inf)) %>%
  unnest(sil) %>%
...
```
#### Statistical evaluation of silhouette plot
```{r}
...
tf_stats = df %>%
  filter(class %in% c("top_2000_seurat_vst","daucellAB","metaviperAB",
                      ...
                      "top_113_seurat_mvg","viperAB", "scenic")) %>%
...
#### Correlation of TF activity scores across scRNA-seq protocols
```{r}
df = readRDS("output/hca_data/summarized_footprint_scores/summarized_footprints_scores.rds") %>%
  filter(!tool %in% c("progeny", "paucell")) %>%
  filter(!celltype %in% c("unclassified", "Megakaryocytes")) %>%
  select(-s, -footprints)

cor_input = df %>%
  unite(key, celltype, feature, remove = F) %>%
  group_by(technology, tool, feature) %>%
  mutate(z = scale(m)) %>%
  ungroup() 

cor_res_high_conf = cor_input %>%
  nest(-tool) %>%
  mutate(cor_df = data %>% map(function(data) {
    data %>%
      filter(confidence %in% c("A","B") | is.na(confidence)) %>%
      select(technology, key, z) %>%
      spread(technology, z, fill=NA)
  })) %>%
  mutate(r = cor_df %>% map(function(cor_df) {
    cor_df %>%
      select_if(is.numeric) %>%
      correlate() %>%
      shave() %>%
...
cor_res_low_conf = cor_input %>%
  nest(-tool) %>%
  mutate(cor_df = data %>% map(function(data) {
    data %>%
      filter(confidence %in% c("C","D", "E") | is.na(confidence)) %>%
      select(technology, key, z) %>%
      spread(technology, z, fill=NA)
  })) %>%
  mutate(r = cor_df %>% map(function(cor_df) {
...
saveRDS(cor_res_high_conf, "output/hca_data/tf_correlations/tf_correlation_high_conf.rds")
saveRDS(cor_res_low_conf, "output/hca_data/tf_correlations/tf_correlation_low_conf.rds")
```
### Blood cancer analysis
#### TF activity vs gene essentiality
```{python}
import pandas as pd
import numpy as np
from scipy.stats import pearsonr as pcor

### read latest HCA data
hca_tf_activity = pd.read_csv('output/hca_data/summarized_footprint_scores/summarized_footprints_scores.csv', 
                              sep=',',header=0,index_col=None)

# remove HEK, Megakaryocytes and unlassified cells and remove the column footprints
hca_tf_activity = hca_tf_activity.drop(hca_tf_activity[(hca_tf_activity.celltype == "HEK cells") | (hca_tf_activity.celltype == "Megakaryocytes") | (hca_tf_activity.celltype == "unclassified") | (hca_tf_activity.tool == "progeny") | (hca_tf_activity.tool == "paucell")].index).drop(["footprints"], axis=1)


### get all tfs in our analysis
all_tfs = hca_tf_activity['feature'].unique()

### read achilles data
achilles = pd.read_csv('data/hca_data/blood_cancer/D2_combined_gene_dep_scores.csv',sep=',',header=0,index_col=0)

### set achilles index to gene names
achilles.index=pd.Series(achilles.index).apply(lambda x:x.split()[0]).values

### get ccle gene exspression values
gex = pd.read_csv('data/hca_data/blood_cancer/CCLE_expression.csv',sep=',',header=0,index_col=0)

### set ccle columns to gene names
gex.columns = pd.Series(gex.columns).apply(lambda x:x.split()[0]).values

### we concentrate on tfs shared in HCA analysis, Achilles and CCLE
shared_tfs = list(set(all_tfs) & set(gex.columns) & set(achilles.index))
gex = gex[shared_tfs]
achilles = achilles.loc[shared_tfs]

cell_anno = pd.read_csv('data/hca_data/blood_cancer/sample_info.csv',sep=',',header=0,index_col=0)

### set gex index to ccle name
gex.index = cell_anno['CCLE Name'][gex.index].values

### shared cell lines between achilles and gex
all_cells = list(set(achilles.columns) & set(gex.index))
gex = gex.loc[all_cells]
achilles = achilles[all_cells].T

gex_achilles_corr=[]
for tf in achilles.columns:
    cells=achilles.index[~achilles[tf].isnull()]
    gex_achilles_corr.append(pcor(achilles.loc[cells,tf],gex.loc[cells,tf])[0])
gex_achilles_corr=pd.DataFrame(pd.Series(gex_achilles_corr,index=achilles.columns))
gex_achilles_corr.columns=['correlation']

gex_achilles_corr.sort_values('correlation')

gex_achilles_corr.to_csv('output/hca_data/blood_cancer/gex_achilles_corr.csv',sep=',')

# multiply achilles with sign of correlation
achilles=achilles*np.sign(gex_achilles_corr['correlation'])
achilles.to_csv('output/hca_data/blood_cancer/tf_achilles_sign_corrected.csv',sep=',')

########
tf_ess = pd.read_csv('output/hca_data/blood_cancer/tf_achilles_sign_corrected.csv',sep=',',header=0,index_col=0)

# Load manually created annotation file for blood cancers. We will use the myeloid_lymphid columns to see the differences between myeloid and lymphid cells.
blood_anno = pd.read_csv('data/hca_data/blood_cancer/blood_cancer_anno.csv',
                         sep=',',header=0,index_col=None)
blood_anno.index=blood_anno['CCLE Name']
fil=blood_anno['myeloid_lymphoid']=='M'
blood_anno.loc[blood_anno.index[fil],'myeloid_lymphoid']='Myeloid'
blood_anno.loc[blood_anno.index[~fil],'myeloid_lymphoid']='Lymphoid'
blood_anno.head()

#make long version of achilles and add annotation
tf_ess = tf_ess.loc[blood_anno.index]
tf_ess['Cell'] = tf_ess.index
tf_ess = tf_ess.melt(id_vars='Cell',value_vars = tf_ess.columns[:-1])
tf_ess.columns=['cell','feature','essentiality']
tf_ess["MorL"] = blood_anno.loc[tf_ess['cell'].values,'myeloid_lymphoid'].values

### we create a similar annotation file for blood cells in HCA data
pbmc_anno = pd.Series({'B cells':'Lymphoid',
...
#add annotation to hca data
hca_tf_activity['MorL']=pbmc_anno[hca_tf_activity['celltype'].values].values

hca_tf_activity.to_csv('output/hca_data/blood_cancer/tf_activities.csv',sep=',')
tf_ess.to_csv('output/hca_data/blood_cancer/tf_essentialities.csv', sep=',')
```

#### Global analysis of difference in effect size of activity and essentiality
```{r}
tf_act = read_csv("output/hca_data/blood_cancer/tf_activities.csv") %>%
  select(-X1)
tf_ess = read_csv("output/hca_data/blood_cancer/tf_essentialities.csv") %>%
  select(-X1)

a = tf_act %>%
  group_by(technology, tool, feature, confidence, MorL) %>%
  summarize(mean_m = mean(m)) %>%
  ungroup() %>%
  spread(MorL, mean_m) %>%
  mutate(diff_activity = Lymphoid - Myeloid) %>%
  select(-Lymphoid, -Myeloid)

e = tf_ess %>%
  group_by(feature, MorL) %>%
  summarise(mean_ess = mean(essentiality, na.rm=T)) %>%
  spread(MorL, mean_ess) %>%
  mutate(diff_ess = Lymphoid - Myeloid) %>%
  select(-Lymphoid, -Myeloid)

res = a %>%
  inner_join(e, by="feature") %>%
  filter(confidence %in% c("A", "B") | is.na(confidence)) %>%
  nest(-c(technology, tool)) %>%
  mutate(r = data %>% map(function(data) {
    data %>%
      select_if(is.numeric) %>%
      cor.test(.$diff_activity, .$diff_ess, data=.) %>%
      tidy() %>%
      select(estimate, p.value, statistic)
  }))

saveRDS(res, "output/hca_data/blood_cancer/effect_size_diff.rds")
```