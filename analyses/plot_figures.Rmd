...
tf_summary = performance %>%
  ...
  scale_fill_gradient(low = rwth_color("petrol25"), 
                      high = rwth_color("petrol"))
```
...
cor = readRDS("output/hca_data/umap/sil_correlation_input.rds") %>%
  ...
  mutate(label = factor(label, levels = c("Seurat (vst)","Pos. control","PROGENy (500)", "P-AUCell (200)", "Neg. control")))

pw_boxplot = cor %>% 
  filter(hrchy_level == "Hrchy. Lvl. 2") %>%
  ggplot(aes(x=label, y=m, fill=label)) +
  geom_jitter(aes(color = label), alpha=0.75) +
  geom_boxplot() +
  coord_flip() +
  theme(legend.position = "none") +
  scale_color_manual(values = rwth_color(c("green", "turquoise","orange", "red", "blue75"))) +
  scale_fill_manual(values = rwth_color(c("green", "turquoise","orange", "red", "blue75"))) +
...
cor = readRDS("output/hca_data/umap/sil_correlation_input.rds") %>%
  ...
  mutate(label = factor(label, levels = c("Seurat (vst)","Pos. control","DoRothEA (AB)", "D-AUCell (AB)", "metaVIPER (AB)", "TF expression (AB)", "Neg. control")))
...
tf_boxplot = cor %>% 
  ...
  coord_flip() +
  theme(legend.position = "none") +
  scale_fill_manual(values = rwth_color(c("green", "orange", "bordeaux", "petrol", "turquoise", "red", "blue75")),drop=F) +
  scale_color_manual(values = rwth_color(c("green", "orange", "bordeaux", "petrol", "turquoise", "red", "blue75")),drop=F) +
  labs(y="Mean silhouette width", x=NULL) +
  my_theme()
```
